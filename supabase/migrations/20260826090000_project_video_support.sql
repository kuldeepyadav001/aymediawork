-- Project video support: optional YouTube URL per work entry so real client
-- work can play on its detail page. The URL is nullable, validated to YouTube
-- watch/short-link formats, and flows through the existing save_admin_project
-- security-invoker function under the established RLS policies.
...
alter table public.projects
  drop constraint if exists projects_video_url_format;
alter table public.projects
  add constraint projects_video_url_format check (
    video_url is null
    or video_url ~ '^https://(www\.)?(youtube\.com/watch\?v=|youtu\.be/)[A-Za-z0-9_-]{6,20}([&?][A-Za-z0-9_=&-]*)?$'
  );
...
begin
  ...
  if requested_id is null then
    insert into public.projects (
      ...
      sort_order, video_url, created_by, updated_by
    ) values (
